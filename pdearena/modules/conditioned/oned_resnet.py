...
from .fourier_cond import SpectralConv1d


class FourierBasicBlock1D(ConditionedBlock):
    expansion: int = 1
    ...
    def __init__(
        ...
        assert not norm
        self.fourier1 = SpectralConv1d(in_planes, planes, cond_channels, modes1=self.modes1)
        self.conv1 = nn.Conv1d(in_planes, planes, kernel_size=1, padding=0, padding_mode="zeros", bias=True)
        self.fourier2 = SpectralConv1d(planes, planes, cond_channels, modes1=self.modes1)
...
class OneD_DilatedBasicBlock(ConditionedBlock):
    ...
    def __init__(
        ...
        self.dilation = [1, 2, 1] #[1, 2, 4, 8, 4, 2, 1]
        dilation_layers = []
        ...
        self.cond_emb = nn.Linear(cond_channels, planes)
        self.last_conv = nn.Conv1d(in_planes,
                    planes,
                    kernel_size=3,
                    stride=stride,
                    dilation=1,
                    padding=1,
                    bias=True,)
        #self.cond_embs = nn.ModuleList([nn.Linear(cond_channels, planes) for i in range(len(self.dilation_layers))])
    ...
    def forward(self, x: torch.Tensor, emb: torch.Tensor) -> torch.Tensor:
        ...
        while len(emb_out.shape) < len(out.shape):
            emb_out = emb_out[..., None]
        out = self.last_conv(out + emb_out)
        return out + x
        ...
        if z is not None:
            if self.param_conditioning.startswith("scalar"):
                ...
                for i in range(z.shape[-1]):
                    emb = self.pde_emb[i](fourier_embedding(z[..., i], self.in_planes))
        # prev = x.float()
